...
namespace turbo::cardano::conway {
    void certificate_t::to_cbor(era_encoder &enc) const
    {
        std::visit([&](const auto &cert) {
            ...
            if constexpr (std::is_same_v<T, stake_reg_cert>) {
                ...
                enc.array(10).uint(3).bytes(cert.pool_id).bytes(params.vrf_vkey)
                    .uint(params.pledge).uint(params.cost);
                ...
                throw error(fmt::format(
                    "certificate type is not supported in Conway: {}", typeid(T).name()));
            }
        }, value.val);
    }
    ...
    void certificates_t::to_cbor(era_encoder &enc) const
    {
        ...
        enc.array_compact(size(), [&] {
            for (const auto &certificate: *this)
