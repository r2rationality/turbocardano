...
namespace fmt {
    template<>
    struct formatter<turbo::cardano::network::multiplexer::ok_t>: formatter<std::span<const uint8_t>> {
    ...
    template<>
    struct formatter<turbo::cardano::network::multiplexer::stopped_t>: formatter<std::span<const uint8_t>> {
        template<typename FormatContext>
        auto format(const auto &, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::network::multiplexer::failed_t>: formatter<std::span<const uint8_t>> {
        template<typename FormatContext>
    ...
    template<>
    struct formatter<turbo::cardano::network::multiplexer::state_t>: formatter<std::span<const uint8_t>> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
