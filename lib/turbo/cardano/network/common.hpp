...
namespace turbo::cardano::network {
    ...
    struct client {
        ...
        void process(turbo::scheduler *sched=nullptr, asio::worker *iow=nullptr)
        {
...
namespace fmt {
    template<>
    struct formatter<turbo::cardano::network::address>: formatter<int> {
    ...
    template<>
    struct formatter<turbo::cardano::network::mini_protocol>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            using turbo::cardano::network::mini_protocol;
            switch (v) {
                ...
                [[unlikely]] default: throw turbo::error(fmt::format("an unsupported value for mini_protocol: {}", static_cast<int>(v)));
            }
        }
    };

    template<>
    struct formatter<turbo::cardano::network::channel_mode>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            using turbo::cardano::network::channel_mode;
            switch (v) {
                ...
                [[unlikely]] default: throw turbo::error(fmt::format("an unsupported value for channel_mode: {}", static_cast<int>(v)));
            }
        }
    };
}
