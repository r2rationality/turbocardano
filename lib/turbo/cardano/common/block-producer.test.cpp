...
suite cardano_block_producer_suite = [] {
    ...
    "cardano::block_producer"_test = [] {
        "one empty block"_test = [] {
            ...
            auto seed = blake2b::digest<ed25519::seed>(std::string_view { "1" });
        ...
        "two producers"_test = [] {
            ...
            auto seed1 = blake2b::digest<ed25519::seed>(std::string_view { "1" });
            ...
            auto seed2 = crypto::blake2b::digest<ed25519::seed>(std::string_view { "2" });
        ...
        "transactions"_test = [] {
            const auto [cold_sk, cold_vk] = ed25519::create_from_seed(crypto::blake2b::digest<ed25519::seed>(std::string_view { "cold-1" }));
            const auto vrf_seed = blake2b::digest<ed25519::seed>(std::string_view { "vrf-1" });
            const auto vrf_sk = vrf03_create_sk_from_seed(vrf_seed);
            block_producer bp { cold_sk, vrf_seed, vrf_sk };

            const auto [acc1_sk, acc1_vk] = ed25519::create_from_seed(crypto::blake2b::digest<ed25519::seed>(std::string_view { "acc-1" }));
            const auto [acc2_sk, acc2_vk] = ed25519::create_from_seed(crypto::blake2b::digest<ed25519::seed>(std::string_view { "acc-2" }));
            ...
            expect_equal(6, blk->era());
            expect_equal(123, blk.offset());
            expect_equal(0, blk->height());
            expect_equal(0, blk->slot());
            expect_equal(1, blk->tx_count());
            blk->foreach_tx([&](const auto &tx) {
                ...
                tx.foreach_input([&](const auto &txi) {
                    ...
                    expect_equal(exp_txi.tx_hash, txi.hash);
                    expect_equal(exp_txi.txo_idx, txi.idx);
                });
                size_t txo_idx = 0;
                tx.foreach_output([&](const auto &txo) {
                    const auto &exp_tx_out = bp.txs.at(tx.index()).outputs.at(txo_idx++);
                    expect_equal(exp_tx_out.address, txo.address_raw);
                    expect_equal(exp_tx_out.coin, txo.coin);
                });
            });
            expect(blk->signature_ok());
        };
