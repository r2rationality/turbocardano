...
suite cardano_common_bench_suite = [] {
    "cardano::common"_test = [] {
        "block method vs direct CBOR access"_test = [] {
            ...
            benchmark("extract slot structured", [&] {
                for (size_t i = 0; i < num_iter; ++i) {
                    blk_ref.slot();
                }
                return num_iter;
            });
            benchmark("extract slot direct",[&] {
                for (size_t i = 0; i < num_iter; ++i) {
        ...
        "static vs dynamic polymorphism"_test = [] {
            ...
            std::vector<static_any> s_objs { static_a {}, static_b {}, static_c {} };
            benchmark("static polymorphism", [&s_objs] {
                uint64_t sum = 0;
                for (size_t i = 0; i < num_iter; ++i) {
            ...
            }, num_iter * s_objs.size());
            std::vector<std::unique_ptr<dynamic_base>> d_objs {};
            ...
            benchmark("dynamic polymorphism", [&d_objs] {
                uint64_t sum = 0;
                for (size_t i = 0; i < num_iter; ++i) {
