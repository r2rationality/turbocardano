...
namespace turbo::cardano {
    ...
    using tx_list = std::vector<tx_base*>;
    ...
    struct block_tx_list {
        ...
        block_tx_list(std::vector<TX> &&t, const buffer &traw, const buffer &wraw):
            txs { std::move(t) },
            raw { traw },
            wits_raw { wraw }
        {
    ...
    struct block_base {
        ...
        {
            return crypto::blake2b::digest<pool_hash>(issuer_vkey());
        }
        ...
        virtual const block_kes_signature kes() const
        {
...
namespace fmt {
    template<>
    struct formatter<turbo::cardano::amount_asset>: formatter<turbo::cardano::amount_pure> {
        template<typename FormatContext>
        auto format(const auto &a, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{} {}/{}", turbo::cardano::amount_pure { a.coins }, a.policy_id, a.asset);
        }
    };

    template<>
    struct formatter<turbo::cardano::balance_change>: formatter<turbo::cardano::amount_pure> {
        template<typename FormatContext>
    ...
    template<>
    struct formatter<turbo::cardano::multi_balance_change>: formatter<turbo::cardano::balance_change> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            auto out_it = ctx.out();
            for (const auto &[asset_name, change]: v) {
                out_it = fmt::format_to(out_it, "{} {}; ", turbo::cardano::balance_change { change }, asset_name);
            }
            return out_it;
        }
    };
    ...
    template<>
    struct formatter<turbo::cardano::tx_base>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            ...
            out_it = fmt::format_to(out_it, "]\n");
            return fmt::format_to(out_it, "total output: {}", turbo::cardano::amount { sum_outputs });
        }
    };

    template<>
    struct formatter<turbo::cardano::nonce>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            if (v)
    ...
    template<>
    struct formatter<turbo::cardano::redeemer_tag>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            using turbo::cardano::redeemer_tag;
            switch (v) {
    ...
    template<>
    struct formatter<turbo::cardano::redeemer_id>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::multi_balance>: formatter<turbo::cardano::multi_balance::base_type> {
    };
}
