...
namespace turbo::cardano {
    ...
    using vkey = crypto::ed25519::vkey;
    using vkey_span = std::span<const uint8_t, sizeof(vkey)>;
    using signature = crypto::ed25519::signature;
    using vrf_nonce = vrf_nonce;
    using vrf_result_span = std::span<const uint8_t, sizeof(vrf_result)>;
    using vrf_proof_span = std::span<const uint8_t, sizeof(vrf_proof)>;
    using cardano_kes_signature_data = byte_array<kes_signature<>::size()>;
    ...
    struct map_t: flat_map<K, V> {
        ...
        const typename base_type::value_type &nth_or_die(const size_t idx) const
        {
            if (const auto it = base_type::nth(idx); it != base_type::end()) [[likely]]
                return *it;
            throw error(fmt::format("map index out of range: {}", idx));
        }
    };
    ...
    struct script_info {
        ...
        {
            if (!_hash) {
                _hash.emplace(crypto::blake2b::digest<script_hash>(_data));
            }
            return *_hash;
        }
        ...
        address(const buffer bytes): _bytes { bytes }
        {
            ...
            switch (type()) {
                ...
                case 0b0111: // enterprise script
                    ...
                    if (_bytes.size() < 29) [[unlikely]]
                        throw error(fmt::format("cardano reward addresses must have at least 29 bytes: {}!", _bytes));
                    break;
                ...
                case 0b0011: // base address: scripthash28,scripthash28
                    ...
                    if (_bytes.size() < 57) [[unlikely]]
                        throw error(fmt::format("shelley base address must have at least 57 bytes: {}!", _bytes));
                    break;
    ...
    using policy_mint_map = map_t<asset_name_t, int64_t>;
    struct multi_mint_map: map_t<script_hash, policy_mint_map> {
        using map_t<script_hash, policy_mint_map>::map_t;
    };
    ...
    using tx_out_ref_list = std::vector<tx_out_ref>;
    ...
    using tx_out_data_list = std::vector<tx_out_data>;
    ...
    struct ex_unit_prices {
        turbo::rational_u64 mem {};
        turbo::rational_u64 steps {};
    ...
    struct pool_voting_thresholds_t {
        turbo::rational_u64 motion_of_no_confidence { 51, 100 };
        turbo::rational_u64 committee_normal { 51, 100 };
        turbo::rational_u64 committee_no_confidence { 51, 100 };
        turbo::rational_u64 hard_fork_initiation { 51, 100 };
        turbo::rational_u64 security_voting_threshold { 51, 100 };
    ...
    struct drep_voting_thresholds_t {
        turbo::rational_u64 motion_no_confidence { 67, 100 };
        turbo::rational_u64 committee_normal { 67, 100 };
        turbo::rational_u64 committee_no_confidence { 67, 100 };
        turbo::rational_u64 update_constitution { 75, 100 };
        turbo::rational_u64 hard_fork_initiation { 67, 100 };
        turbo::rational_u64 pp_network_group { 67, 100 };
        turbo::rational_u64 pp_economic_group { 67, 100 };
        turbo::rational_u64 pp_technical_group { 67, 100 };
        turbo::rational_u64 pp_governance_group { 75, 100 };
        turbo::rational_u64 treasury_withdrawal { 67, 100 };
    ...
    struct param_update {
        ...
        {
            crypto::blake2b::digest(hash, bytes);
        }
...
namespace fmt {
    template<>
    struct formatter<turbo::cardano::epoch>: formatter<uint64_t> {
    ...
    template<>
    struct formatter<turbo::cardano::asset_name_t>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const turbo::cardano::asset_name_t &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{}", turbo::buffer_readable { v.span() });
        }
    };

    template<>
    struct formatter<turbo::cardano::protocol_version>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::slot>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::tx_out_idx>: formatter<uint64_t> {
        template<typename FormatContext>
    ...
    template<>
    struct formatter<turbo::cardano::stake_ident>: formatter<uint64_t> {
        template<typename FormatContext>
    ...
    template<>
    struct formatter<turbo::cardano::pay_ident>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &id, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::amount_pure> {
    ...
    template<>
    struct formatter<turbo::cardano::amount>: formatter<turbo::cardano::amount_pure> {
        template<typename FormatContext>
        auto format(const auto &a, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{} ADA", turbo::cardano::amount_pure { a.coins });
        }
    };

    template<>
    struct formatter<turbo::cardano::point2>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::point3>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::point>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::tx_out_ref>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::datum_option_t>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::tx_out_data>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            if (!v.address_raw.empty()) [[likely]] {
                return fmt::format_to(ctx.out(), "address: #{} ({}) coin: {} assets: {} datum: {} script_ref: {}",
                    v.address_raw, turbo::cardano::address { v.address_raw }, turbo::cardano::amount { v.coin }, v.assets, v.datum, v.script_ref);
            }
    ...
    template<>
    struct formatter<turbo::cardano::stake_pointer>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::stake_ident_hybrid>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            switch (v.index()) {
                case 0:
                    return fmt::format_to(ctx.out(), "{}", std::get<turbo::cardano::stake_ident>(v));
    ...
    template<>
    struct formatter<turbo::cardano::shelley_delegate>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::script_type>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const turbo::cardano::script_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            using turbo::cardano::script_type;
            switch (v) {
    ...
    template<>
    struct formatter<turbo::cardano::byron_addr>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::address>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const turbo::cardano::address &addr, FormatContext &ctx) const -> decltype(ctx.out()) {
            using turbo::buffer;
            switch (addr.type()) {
                case 0b1000:
    ...
    template<>
    struct formatter<turbo::cardano::protocol_params>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::ex_unit_prices>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::ex_units>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "mem: {} steps: {}", v.mem, v.steps);
        }
    ...
    template<>
    struct formatter<turbo::cardano::param_update>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::param_update_proposal>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::param_update_vote>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
        struct formatter<turbo::cardano::drep_voting_thresholds_t>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const turbo::cardano::drep_voting_thresholds_t &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::pool_voting_thresholds_t>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const turbo::cardano::pool_voting_thresholds_t &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(
                ctx.out(),
    ...
    template<>
    struct formatter<turbo::cardano::slot_range>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::drep_t>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return std::visit([&](const auto &cred) {
                using T = std::decay_t<decltype(cred)>;
                if constexpr (std::is_same_v<T, turbo::cardano::credential_t>)
                    return fmt::format_to(ctx.out(), "{}", cred);
                if constexpr (std::is_same_v<T, turbo::cardano::drep_t::abstain_t>)
                    return fmt::format_to(ctx.out(), "abstain");
                if constexpr (std::is_same_v<T, turbo::cardano::drep_t::no_confidence_t>)
                    return fmt::format_to(ctx.out(), "no-confidence");
                throw turbo::error(fmt::format("unsupported drep.type: {}", typeid(v).name()));
    ...
    template<>
    struct formatter<turbo::cardano::plutus_cost_model>: formatter<turbo::cardano::plutus_cost_model::storage_type> {
    };

    template<>
    struct formatter<turbo::cardano::optional_slot>: formatter<std::optional<uint64_t>> {
    };

    template<typename T>
    struct formatter<turbo::cardano::optional_t<T>>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            if (v)
    ...
    template<>
    struct formatter<turbo::cardano::relay_info>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            switch (v.val.index()) {
                case 0: {
                    const auto &ra = std::get<turbo::cardano::relay_addr>(v.val);
                    return fmt::format_to(ctx.out(), "port: {} ipv4: {} ipv6: {}", ra.port, ra.ipv4, ra.ipv6);
                }
                case 1: {
                    const auto &rh = std::get<turbo::cardano::relay_host>(v.val);
    ...
    template<>
    struct formatter<turbo::cardano::pool_metadata>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::pool_params>: formatter<uint64_t> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    template<>
    struct formatter<turbo::cardano::script_info>: formatter<int> {
        template<typename FormatContext>
        auto format(const auto &v, FormatContext &ctx) const -> decltype(ctx.out()) {
    ...
    struct formatter<turbo::cardano::map_t<K, V>>: formatter<int> {
        template<typename FormatContext>
        auto format(const typename turbo::cardano::map_t<K, V>::base_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{}", v);
        }
    };

    template<>
    struct formatter<turbo::cardano::multi_asset_map>: formatter<int> {
        template<typename FormatContext>
        auto format(const turbo::cardano::multi_asset_map::base_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{}", v);
        }
    };

    template<>
    struct formatter<turbo::cardano::reward_id_t>: formatter<int> {
        template<typename FormatContext>
        auto format(const turbo::cardano::reward_id_t &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "reward_id hash: {} script: {}", v.hash(), v.script());
        }
    };

    template<typename T>
    struct formatter<turbo::cardano::vector_t<T>>: formatter<int> {
        template<typename FormatContext>
        auto format(const typename turbo::cardano::vector_t<T>::base_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{}", v);
        }
    };

    template<typename T>
    struct formatter<turbo::cardano::nil_optional_t<T>>: formatter<int> {
        template<typename FormatContext>
        auto format(const typename turbo::cardano::nil_optional_t<T>::base_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{}", v);
        }
    };

    template<typename T>
    struct formatter<turbo::cardano::array_optional_t<T>>: formatter<int> {
        template<typename FormatContext>
        auto format(const typename turbo::cardano::array_optional_t<T>::base_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{}", v);
        }
    };

    template<typename T>
    struct formatter<turbo::cardano::prefix_optional_t<T>>: formatter<int> {
        template<typename FormatContext>
        auto format(const typename turbo::cardano::prefix_optional_t<T>::base_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{}", v);
        }
    };

    template<>
    struct formatter<turbo::cardano::stake_keyhash_t>: formatter<int> {
        template<typename FormatContext>
        auto format(const turbo::cardano::stake_keyhash_t::base_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
            return fmt::format_to(ctx.out(), "{}", v);
        }
    };
}
...
namespace std {
    ...
    template<>
    struct hash<turbo::cardano::stake_pointer> {
    ...
    template<>
    struct hash<turbo::cardano::stake_ident> {
        size_t operator()(const auto &stake_id) const noexcept
        {
            return turbo::buffer { stake_id.hash.data(), 8 }.to<size_t>();
        }
    };

    template<>
    struct hash<turbo::cardano::stake_ident_hybrid> {
        size_t operator()(const auto &id) const noexcept
        {
            if (holds_alternative<turbo::cardano::stake_ident>(id))
                return hash<turbo::cardano::stake_ident> {} (get<turbo::cardano::stake_ident>(id));
            if (holds_alternative<turbo::cardano::stake_pointer>(id))
                return hash<turbo::cardano::stake_pointer> {} (get<turbo::cardano::stake_pointer>(id));
            return 0;
        }
    };

    template<>
    struct hash<turbo::cardano::param_update> {
        size_t operator()(const auto &v) const noexcept
        {
            return turbo::buffer { v.hash.data(), 8 }.to<size_t>();
        }
    };
}
