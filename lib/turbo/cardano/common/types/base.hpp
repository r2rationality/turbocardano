...
namespace turbo::cardano {
    ...
    struct set_t: flat_set<T> {
        ...
        const T &nth_or_die(size_t idx) const
        {
            if (const auto it = base_type::nth(idx); it != base_type::end())
                return *it;
...
namespace fmt {
    template<typename T>
    struct formatter<turbo::cardano::set_t<T>>: formatter<int> {
        template<typename FormatContext>
        auto format(const typename turbo::cardano::set_t<T>::base_type &v, FormatContext &ctx) const -> decltype(ctx.out()) {
