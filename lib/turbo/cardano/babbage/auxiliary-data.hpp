...
#include <turbo/cardano/alonzo/auxiliary-data.hpp>

namespace turbo::cardano::babbage {
    using alonzo::auxiliary_data_array_t;
    using alonzo::metadata_t;

    struct auxiliary_data_map_t: alonzo::auxiliary_data_map_t {
        std::vector<script_info> plutus_v2_scripts {};

        static auxiliary_data_map_t from_cbor(cbor::zero2::value &);

        static constexpr auto serialize(auto &archive, auto &self)
        {
            return archive(self.metadata, self.native_scripts, self.plutus_v1_scripts, self.plutus_v2_scripts);
        }
    };
