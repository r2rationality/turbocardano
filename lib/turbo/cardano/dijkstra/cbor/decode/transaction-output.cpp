...
namespace turbo::cardano::dijkstra {
    namespace {
        transaction_output_t from_array(cbor::zero2::value &v)
        {
            ...
            if (!it.done()) [[unlikely]]
                throw error("unexpected trailing Dijkstra transaction_output elements");
    ...
    transaction_output_t transaction_output_t::from_cbor(cbor::zero2::value &v)
    {
        ...
        return { babbage::detail::transaction_output_map_from_cbor(
            v,
            [](auto &value) { return std::move(value_t::from_cbor(value).value); },
            [](auto &script) { return std::move(script_t::from_cbor(script).value); }) };
    }
