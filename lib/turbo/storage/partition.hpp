...
namespace turbo::storage {
    struct partition {
        using storage_type = std::vector<const chunk_info *>;
    ...
    private:
        std::vector<const chunk_info *> _chunks {};
    };
    ...
    private:
        const std::vector<partition> _parts;

        static std::vector<partition> _make_partitions(const chunk_registry &cr);
    };
    ...
    private:
        ...
        static std::vector<partition> _make_partitions(const chunk_registry &cr, std::optional<uint64_t> from_slot, std::optional<uint64_t> to_slot);
    };
