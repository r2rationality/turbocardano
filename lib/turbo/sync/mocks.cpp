...
namespace turbo::sync {
    ...
    {
        const auto seed1 = crypto::blake2b::digest<crypto::ed25519::seed>(std::string_view { "1" });
        const auto sk1 = crypto::ed25519::create_sk_from_seed(seed1);
        const auto vrf_sk1 = vrf03_create_sk_from_seed(seed1);
        const auto seed2 = crypto::blake2b::digest<crypto::ed25519::seed>(std::string_view { "2" });
        const auto sk2 = crypto::ed25519::create_sk_from_seed(seed2);
        ...
        cfg.emplace("config", json::object {
            ...
            { "ByronGenesisHash", fmt::format("{}", crypto::blake2b::digest<block_hash>(json::serialize_canon(cfg.at("byron-genesis").json()))) },
            { "ShelleyGenesisFile", "shelley-genesis" },
            { "ShelleyGenesisHash", fmt::format("{}", crypto::blake2b::digest<block_hash>(cfg.at("shelley-genesis").bytes())) },
            { "AlonzoGenesisFile", "alonzo-genesis" },
            { "AlonzoGenesisHash", fmt::format("{}", crypto::blake2b::digest<block_hash>(cfg.at("alonzo-genesis").bytes())) },
            { "ConwayGenesisFile", "conway-genesis" },
            { "ConwayGenesisHash", fmt::format("{}", crypto::blake2b::digest<block_hash>(cfg.at("conway-genesis").bytes())) }
        });
        ...
        chain.data_hash = crypto::blake2b::digest<block_hash>(chain.data);
        return chain;
    }

    void write_turbo_metadata(const std::string &www_dir, const mock_chain &chain, const crypto::ed25519::skey &sk)
    {
        ...
        struct epoch {
            uint8_vector data {};
            std::vector<const parsed_block *> blocks {};
        ...
        for (size_t epoch_no = 0; epoch_no < epochs.size(); ++epoch_no) {
            ...
            const auto data_hash = crypto::blake2b::digest<block_hash>(epoch.data);
