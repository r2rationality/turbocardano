...
suite crypto_ed25519_bench_suite = [] {
    "crypto::ed25519"_test = [] {
        ...
        benchmark(std::string { name } + "/raw", [&] {
            for (size_t i = 0; i < num_evals; ++i) {
                ed25519::verify(sig, vk, msg);
            }
        }, num_evals);
        benchmark(std::string { name } + "/raw-invalid", [&] {
            for (size_t i = 0; i < num_evals; ++i) {
                ed25519::verify(sig_invalid, vk, msg);
            }
        }, msg.size() *num_evals);
        benchmark(std::string { name } + "/hash", [&] {
            blake2b::hash_32 hash;
            for (size_t i = 0; i < num_evals; ++i) {
                blake2b::digest(hash, msg);
                ed25519::verify(sig, vk, hash);
            }
        }, num_evals);
        size_t num_evals_par = 100;
        benchmark(name + std::string { "-parallel" }, [&] {
